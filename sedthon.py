...
async def join_channel():
    try:
        await sedthon(JoinChannelRequest("@Source_elesyed"))
...
**♔ 𝙺𝙸𝙽𝙶𝚂 ɪѕ ᴡᴏʀɪɴɢ⛧
╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
❖╎𝚆𝙴𝙻𝙺𝙾𝙼𝙴 𝚃𝙾 ѕᴏᴜʀᴄᴇ ᴏғ ᴋɪɴɢѕ𐇮
❖╎𝐝𝐞𝐯 : 𝙷𝙰𝚂𝙰𝙽⚚
❖╎𝐜𝐡𝐚𝐧𝐧𝐞𝐥 : @source_of_kingis
❖╎𝐯𝐞𝐫𝐬𝐢𝐨𝐧 : 2.7 - revised 
❖╎ɪᴅ: {event.sender_id}
╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
''')
...
async def _(event):
    ...
    البايو : `{bio}`
        ''', reply_to=event)
            ...
            await sedthon.send_message(event.chat_id, f"ɪᴅ𐇮 : `{event.sender_id}`")
    else:
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('اهلا مطوري الاسياد - @IC_X_K')
